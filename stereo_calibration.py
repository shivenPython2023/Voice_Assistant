...
for imgLeft, imgRight in zip(imagesLeft, imagesRight):
    ...
    print(f"Left: {imgLeft}, Right: {imgRight}")
    ...
    print(f"Chessboard detected: {retL}")
    retR, cornersR = cv.findChessboardCorners(grayR, chessboardSize, None)
    print(f"Chessboard detected: {retR}")

    print(f"Number of valid chessboard detections: {len(imgpointsL)}")
    ...
    if retL and retR == True:
        ...
        string = 'img left'+ str(count)
        string2 = 'img right'+ str(count)

        cv.imshow(string, imgL)
        cv.drawChessboardCorners(imgR, chessboardSize, cornersR, retR)
        cv.imshow(string2, imgR)
        cv.waitKey(1000)
