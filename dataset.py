...
num_delete= 0
def is_valid_audio(file_path, min_duration=0.2, min_volume=-45):
    try:
        waveform, sr = torchaudio.load(file_path)
        duration = waveform.shape[1] / sr  # Compute duration in seconds
        
        if duration < min_duration:
            return False  # Too short

        # Compute volume (RMS-based estimate)
        volume_db = 20 * torch.log10(torch.sqrt((waveform**2).mean()))
...
class WakeWordData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Load waveform (disable automatic normalization)
            waveform, sr = torchaudio.load(file_path, normalize=False)

            # Convert int16 to float32 if necessary
            if waveform.dtype == torch.int16:
                waveform = waveform.to(torch.float32) / 32768.0  # Normalize to [-1, 1]
            ...
            # Ensure MFCC is valid
            if mfcc.shape[-1] == 0:
                raise ValueError(f"Invalid MFCC shape: {mfcc.shape} for file {file_path}")
